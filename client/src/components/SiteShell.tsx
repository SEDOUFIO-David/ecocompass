      ...
      <header className="sticky top-0 z-50 border-b border-[#14333A]/10 bg-[#F8F5ED]/92 backdrop-blur-xl">
        <div className="container flex h-[74px] items-center justify-between gap-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/mon-ecocompass" className="hidden items-center gap-2 rounded-full bg-[#0E6356] px-4 py-2.5 text-sm font-semibold text-white shadow-[0_9px_24px_rgba(14,99,86,.2)] transition hover:-translate-y-0.5 hover:bg-[#0A5147] lg:flex">Mon EcoCompass <UserRound size={16} /></Link>
        ...
        {menuOpen && <nav className="border-t border-[#14333A]/10 bg-[#F8F5ED] px-4 py-4 xl:hidden" aria-label="Navigation mobile">
          <div className="mx-auto flex max-w-2xl flex-col gap-1">
            ...
            <div className="mt-2 grid gap-2 sm:grid-cols-2"><Link href="/mon-ecocompass" className="flex items-center gap-2 rounded-xl border border-[#14333A]/12 px-4 py-3 font-medium"><UserRound size={17} /> Mon EcoCompass</Link><Link href="/mon-parcours" className="flex items-center gap-2 rounded-xl border border-[#14333A]/12 px-4 py-3 font-medium"><Compass size={17} /> Mon parcours</Link><Link href="/plan-action" className="flex items-center gap-2 rounded-xl border border-[#14333A]/12 px-4 py-3 font-medium"><FlaskConical size={17} /> Mon plan d’action</Link><Link href="/portfolio" className="flex items-center gap-2 rounded-xl border border-[#14333A]/12 px-4 py-3 font-medium"><BookmarkCheck size={17} /> Mon portfolio</Link><Link href="/favoris" className="flex items-center gap-2 rounded-xl border border-[#14333A]/12 px-4 py-3 font-medium"><BookmarkCheck size={17} /> Mes favoris</Link></div>
          </div>
        </nav>}
      </header>
