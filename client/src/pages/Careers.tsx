...
import { ArrowRight, BriefcaseBusiness, CircleAlert, Compass, Scale, Wrench } from "lucide-react";
...
export default function Careers() {
  ...
  return <SiteShell>
    ...
    <section className="bg-white py-7"><div className="container flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex gap-2 overflow-x-auto pb-1">{categories.map((item) => <button type="button" onClick={() => setCategory(item)} className={`filter-pill ${category === item ? "filter-pill-active" : ""}`} key={item}>{item}</button>)}</div><Link href="/metiers/comparer" className="compare-link"><Scale size={16} /> Comparer des métiers</Link></div></section>
