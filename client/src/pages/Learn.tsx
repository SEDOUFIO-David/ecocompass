...
export default function Learn() {
  ...
  const filtersActive = Boolean(query) || level !== "Tous les niveaux" || domain !== "Tous les domaines";
  const clearFilters = () => { setQuery(""); setLevel("Tous les niveaux"); setDomain("Tous les domaines"); };
  return <SiteShell>
    ...
    <section className="bg-white py-9"><div className="container ui-control-bar"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#E2EEE7] text-[#0E6356]"><Filter size={19} /></span><div><p className="text-sm font-semibold text-[#123139]">Affiner les cours</p><p className="text-xs text-[#6C7D79]" aria-live="polite">{displayed.length} cours correspondent à votre sélection</p></div></div><div className="ui-filter-controls"><label className="filter-field filter-search"><span><Search size={15} /> Rechercher</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Titre, notion ou domaine" aria-label="Rechercher un cours" /></label><label className="filter-field"><span>Niveau</span><select value={level} onChange={(event) => setLevel(event.target.value)}><option>Tous les niveaux</option>{levels.map((item) => <option key={item}>{item}</option>)}</select></label><label className="filter-field"><span>Domaine</span><select value={domain} onChange={(event) => setDomain(event.target.value)}><option>Tous les domaines</option>{learningDomains.map((item) => <option key={item}>{item}</option>)}</select></label>{filtersActive && <button type="button" className="filter-reset" onClick={clearFilters}>Effacer les filtres</button>}</div></div></section>
      <section className="bg-[#F8F5ED] py-12 lg:py-16"><div className="container">{displayed.length > 0 ? <div className="learning-atlas">{chapters.map(([chapter, chapterCourses], chapterIndex) => <section className="course-chapter" key={chapter}><div className="chapter-margin"><span className="chapter-number">0{chapterIndex + 1}</span><div><p className="eyebrow">Niveau {chapter}</p><h2>{chapterText[chapter][0]}</h2><p>{chapterText[chapter][1]}</p></div><Link href="/glossaire" className="chapter-source"><Compass size={17} /><span>Notions liées</span></Link></div><div className="course-cluster">{chapterCourses.map((course, courseIndex) => <article className={`course-card course-card-atlas ${courseIndex === 0 ? "course-card-featured course-card-route-anchor" : "course-card-route-stop"}`} key={course.id}><FavoriteButton compact item={{ id: `course:${course.slug}`, type: "course", title: course.title, href: `/apprendre/${course.slug}` }} /><Link href={`/apprendre/${course.slug}`} className="course-card-link"><div className="flex items-start justify-between gap-4"><span className="course-number">{String(course.id).padStart(2, "0")}</span><span className="level-chip">{course.level}</span></div><p className="mt-8 font-mono text-xs text-[#6A7D78]">{course.domain}</p><h2>{course.title}</h2><p>{course.summary}</p><div className="course-competency-preview"><span>À exercer</span><b>{course.competencies[0].label}</b></div><div className="course-card-footer"><span><BookOpen size={16} /> {course.duration}</span><span className="card-arrow">Lire la notion <ArrowRight size={16} /></span></div></Link></article>)}</div></section>)}</div> : <div className="empty-state" role="status"><BookOpen size={28} /><h2>Aucun cours ne correspond à ces critères.</h2><p>Essayez un autre mot-clé ou réinitialisez les filtres pour retrouver tous les parcours disponibles.</p><button type="button" className="cta-secondary mt-3" onClick={clearFilters}>Réinitialiser la recherche et les filtres</button></div>}</div></section>
  </SiteShell>;
}
