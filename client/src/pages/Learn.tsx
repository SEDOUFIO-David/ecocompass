...
export default function Learn() {
  ...
  return <SiteShell>
    ...
    <section className="bg-[#F8F5ED] py-12 lg:py-16"><div className="container">{displayed.length > 0 ? <div className="learning-atlas">{chapters.map(([chapter, chapterCourses], chapterIndex) => <section className="course-chapter" key={chapter}><div className="chapter-margin"><span className="chapter-number">0{chapterIndex + 1}</span><div><p className="eyebrow">Niveau {chapter}</p><h2>{chapterText[chapter][0]}</h2><p>{chapterText[chapter][1]}</p></div><Link href="/glossaire" className="chapter-source"><Compass size={17} /><span>Notions liées</span></Link></div><div className="course-cluster">{chapterCourses.map((course, courseIndex) => <Link href={`/apprendre/${course.slug}`} className={`course-card course-card-atlas ${courseIndex === 0 ? "course-card-featured course-card-route-anchor" : "course-card-route-stop"}`} key={course.id}><div className="flex items-start justify-between gap-4"><span className="course-number">{String(course.id).padStart(2, "0")}</span><span className="level-chip">{course.level}</span></div><p className="mt-8 font-mono text-xs text-[#6A7D78]">{course.domain}</p><h2>{course.title}</h2><p>{course.summary}</p><div className="course-card-footer"><span><BookOpen size={16} /> {course.duration}</span><span className="card-arrow">Ouvrir <ArrowRight size={16} /></span></div></Link>)}</div></section>)}</div> : <div className="empty-state"><BookOpen size={28} /><h2>Aucun cours pour cette combinaison.</h2><p>Essayez un autre mot-clé ou modifiez les filtres pour retrouver les parcours disponibles.</p><button type="button" className="cta-secondary mt-3" onClick={() => { setQuery(""); setLevel("Tous les niveaux"); setDomain("Tous les domaines"); }}>Réinitialiser la recherche et les filtres</button></div>}</div></section>
  </SiteShell>;
}
