...
import { getEconomicSearchTerms, getEconomicSynonymMatches } from "@/data/economicSynonyms";
...
const resultsIndex: Result[] = [
  ...courses.map((course) => ({ type: "Cours" as const, title: course.title, description: course.summary, href: `/apprendre/${course.slug}`, keywords: [course.domain, course.level, ...course.notions, ...course.competencies.flatMap((competency) => [competency.label, competency.criterion])]})),
...
export default function SearchPage() {
  ...
  const synonymMatches = useMemo(() => getEconomicSynonymMatches(query), [query]);
  const searchTerms = useMemo(() => getEconomicSearchTerms(query), [query]);
  const results = useMemo(() => resultsIndex.filter((item) => { const haystack = [item.title, item.description, ...item.keywords].join(" ").toLocaleLowerCase("fr").normalize("NFD").replace(/[\u0300-\u036f]/g, ""); return (!cleaned || searchTerms.some((term) => haystack.includes(term))) && (active === "Tous" || item.type === active); }), [cleaned, searchTerms, active]);
