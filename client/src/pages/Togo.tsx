...
export default function Togo() {
  ...
  return <SiteShell>
    ...
    <section className="bg-[#F8F5ED] py-12"><div className="container"><div className="mb-8 flex items-end justify-between gap-5"><div><p className="eyebrow">Repères de tableau de bord</p><h2 className="font-display text-4xl tracking-[-.05em]">Indicateurs à documenter</h2></div><span className="font-mono text-xs text-[#71827D]">Unité + année + source requis</span></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{indicators.map(([label, value, note]) => <article className="indicator-card" key={label}><p>{label}</p><b>{value}</b><small>{note}</small><span className="data-trace">STATUT: démo · PÉRIODE: 2024 · SOURCE: simulation</span></article>)}</div></div></section>
